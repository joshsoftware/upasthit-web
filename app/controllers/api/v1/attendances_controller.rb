  ...
  module V1
    class AttendancesController < V1::BaseController
      ...
      def sms_callback_pinnacle
        #same service can be used but make data compatible to it.
        # sms_callback_params = parse_message
        # if create_service(sms_callback_params).create
        #   render json: create_service.result
        # else
      ...
      def validate_api_auth_token
        render json: {message: I18n.t("error.invalid_token")}, status: 401 unless Figaro.env.API_AUTH_TOKEN_VALUE === request.headers[Figaro.env.API_AUTH_TOKEN]
      end
    end
  end
end
